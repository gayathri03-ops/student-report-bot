from app import nlu, safe_query, explain, retrieval, logging_store
from app.access_control import AccessContext, AccessDenied, check_access
...
def answer_question(question: str, role: str = "admin", own_reg_no: str | None = None) -> dict:
    context = AccessContext(role=role, own_reg_no=own_reg_no)
    ...
    try:
        if parsed.intent == "get_result":
            if not parsed.reg_no:
                ...
                check_access(parsed.intent, parsed.reg_no, context)
            ...
            check_access(parsed.intent, None, context)
        ...
        if needs_retrieval_fallback:
            ...
            if matches and matches[0]["score"] >= RETRIEVAL_CONFIDENCE_THRESHOLD:
                best = matches[0]
                check_access("retrieve_context", best["reg_no"], context)
                record = safe_query.get_student_result(best["reg_no"])
                if record:
                    data = record
                    citation = (
                        f"[matched record: {record['reg_no']} - "
                        f"{record['name']}, confidence {best['score']}]"
                    )
                    answer = explain.explain_result(record) + f"\n\n{citation}"
                    parsed.intent = "retrieve_context"
                else:
                    answer = (
                        "I couldn't confidently match that question to a "
                        "student record. Try including a register number, "
                        "or ask about results, attendance, top performers, "
                        "failed students, or the class summary."
                    )
            else:
                answer = (
                    "I can help with: your result (give your register "
                    "number), attendance, class averages, top performers, "
                    "failed students, or an overall class summary. Try "
                    "asking something like "
                    "'What is my result for 411723106003?'"
                )
 
    except AccessDenied as e:
        answer = f"Access denied: {e}"
        parsed.intent = "access_denied"
 
    answer = explain.maybe_llm_polish(answer, question)
 
    logging_store.log_interaction(
        question=question, intent=parsed.intent, role=role,
        own_reg_no=own_reg_no, answer=answer,
    )
